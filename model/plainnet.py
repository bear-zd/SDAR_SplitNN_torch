...
class FModel(nn.Module):
    def __init__(self, level, input_shape, width="standard"):
        ...
        for i in range(level, 0, -1):
            ...
            if i == 0:
                in_c = input_shape[0]
            self.layers.insert(0, BasicBlock(in_c, out_c, 2 if i in [4, 7] else 1))
...
class GModel(nn.Module):
    def __init__(self, level, input_shape, num_classes=10, dropout=0.0, width="standard"):
        ...
        for i in range(8, level-1, -1):
            in_c, out_c  = layer_config[i]
            self.layers.insert(0, BasicBlock(in_c if i != level else input_shape[0], out_c, 2 if i in [3, 6] else 1))
